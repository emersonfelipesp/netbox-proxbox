...
Firecracker support lets API clients provision lightweight micro-VMs without modeling them as normal QEMU VMs or LXC containers. The Proxbox plugin owns the NetBox-side inventory that makes the runtime selectable and auditable.
...
```mermaid
sequenceDiagram
    participant Client as Cloud API client
    participant Backend as Orchestration service
    participant Plugin as netbox-proxbox API
    participant Proxbox as proxbox-api
    participant Agent as Firecracker host-agent

    Client->>Backend: POST /cloud/instances runtime=firecracker
    ...
    Backend-->>Client: kind=firecracker, instance_ref=firecracker:<id>
```
...
- The plugin does not install or run Firecracker itself; it records inventory and exposes APIs used by external clients and `proxbox-api`.
