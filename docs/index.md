...
The `0.0.23.post1` release pairs with a `proxbox-api` build containing the
guest-VM-interface writer and makes `guest_os_model` the default for new and
existing installs. The historical `0.0.23` row remains documented below because
that release introduced the dual VM interface model.
