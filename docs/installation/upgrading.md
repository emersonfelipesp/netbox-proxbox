...
- Proxbox `0.0.15` is the current release for NetBox `4.5.8`, `4.5.9`, and `4.6.x` (certified simultaneously against `v4.5.8`, `v4.5.9`, and official `v4.6.0`; declared compatibility range `4.5.8` through `4.6.99`). It is certified with the separate `proxbox-api` backend release `0.0.11`. `proxbox-api` is not installed as a plugin dependency. The previous `0.0.14` release pairs with backend `0.0.10.post2`.
- This release adds two new migrations: `0038_fastapiendpoint_use_https` (decouples HTTPS scheme from `verify_ssl`) and `0039_pluginsettings_overwrite_ip_address_dns_name` (adds the new IP-DNS overwrite flag to both `ProxboxPluginSettings` and `ProxmoxEndpoint`). Run `python manage.py migrate netbox_proxbox` after upgrade.
...
- Review the release notes before jumping from older `0.0.7` or early `0.0.9` installs; the `0.0.15` line continues the NetBox `4.5.8` / `4.5.9` / `4.6` compatibility path established in `0.0.13.post4` and `0.0.14`.
