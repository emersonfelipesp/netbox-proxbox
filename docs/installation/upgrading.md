...
- Proxbox `0.0.19` is the current release for NetBox `4.5.8`, `4.5.9`, and `4.6.x` (validated against `v4.5.8`, `v4.5.9`, `v4.6.0`, and official `v4.6.1`; declared compatibility range `4.5.8` through `4.6.99`). It pairs with the separate `proxbox-api` backend release `0.0.16`. The previous `0.0.18`/`0.0.18.post1` release pairs with backend `0.0.14`.
