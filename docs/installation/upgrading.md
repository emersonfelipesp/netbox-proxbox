...
- Proxbox `0.0.16` is the current release for NetBox `4.5.8`, `4.5.9`, and `4.6.x` (certified simultaneously against `v4.5.8`, `v4.5.9`, and official `v4.6.0`; declared compatibility range `4.5.8` through `4.6.99`). It is certified with the separate `proxbox-api` backend release `0.0.11`, with the matching `/intent/*` HTTP surface shipping in backend `0.0.12`. `proxbox-api` is not installed as a plugin dependency. The previous `0.0.15` release pairs with backend `0.0.11`.
