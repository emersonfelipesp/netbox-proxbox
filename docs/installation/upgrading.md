...
- Proxbox `0.0.25` is the current release for NetBox `4.5.8` through `4.5.10` and `4.6.x` (validated against `v4.5.8` through `v4.5.10` and `v4.6.0` through `v4.6.6`; declared stable range `4.5.8` through `4.6.99`), plus experimental evaluation on exact canonical NetBox `4.7.0-beta2`. The numeric `max_version` is `4.7.0`; a fail-closed canonical metadata guard rejects later prereleases and GA identities while allowing NetBox startup to omit the plugin. It pairs with `proxbox-api 0.0.20`, `proxmox-sdk 0.0.13`, and `netbox-sdk 0.0.10`. The previous stable `0.0.23.post2` release introduced bounded endpoint auto-configuration.
