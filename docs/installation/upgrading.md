...
- Proxbox `0.0.16` is the current release for NetBox `4.5.8`, `4.5.9`, and `4.6.x` (certified simultaneously against `v4.5.8`, `v4.5.9`, and official `v4.6.0`; declared compatibility range `4.5.8` through `4.6.99`). It is certified with the separate `proxbox-api` backend release `0.0.12`, which ships the matching `/intent/*` HTTP surface. `proxbox-api` is not installed as a plugin dependency. The previous `0.0.15` release pairs with backend `0.0.11`; the older `0.0.14` release pairs with backend `0.0.10.post2`.
- This release adds four new migrations for the opt-in NetBox→Proxmox intent path: `0038_intent_permissions` (seven RBAC permissions), `0039_intent_custom_fields` (10 VM CFs and 2 Branch CFs, with the Branch CFs guarded so they are a no-op when `netbox_branching` is not installed), `0040_apply_job_full` (full `ProxmoxApplyJob` schema), and `0041_deletion_request_full` (full `DeletionRequest` schema). Run `python manage.py migrate netbox_proxbox` after upgrade. With `netbox_to_proxmox_enabled=False` (default), no behavior changes from the read-only reflection path.
...
- Review the release notes before jumping from older `0.0.7` or early `0.0.9` installs; the `0.0.16` line continues the NetBox `4.5.8` / `4.5.9` / `4.6` compatibility path established in `0.0.13.post4`, `0.0.14`, and `0.0.15`. For `0.0.16` specifically, the new NetBox→Proxmox intent path is opt-in via `ProxboxPluginSettings.netbox_to_proxmox_enabled` plus a typed-confirmation phrase; nothing changes for existing installs unless an operator explicitly opts in.
