...
- Proxbox `0.0.27rc14` source supports NetBox `4.5.8` through `4.7.0`, including official v4.7.0 GA. It is validated against `v4.5.8` through `v4.5.10`, `v4.6.0` through `v4.6.6`, and exact v4.7.0 source commit `5f06007e4c9bacc93ce17c1e645fc1143d60df3d`. The current sibling source and released runtime stack is `proxbox-api 0.0.23.post1`, `proxmox-sdk 0.0.15`, and `netbox-sdk 0.0.13`. The previous stable `0.0.23.post2` plugin release introduced bounded endpoint auto-configuration.
...
> **Current source:** netbox-proxbox `0.0.27rc14` supports NetBox `4.5.8`-`4.7.0` stable, including v4.7.0 GA. Current source pairing: netbox-proxbox 0.0.27rc14 <-> proxbox-api 0.0.23.post1 <-> proxmox-sdk 0.0.15 <-> netbox-sdk 0.0.13. The current released runtime pairing uses the same backend stack. The netbox-sdk dependency is REST-only and does not provide the semantic MCP bridge.
