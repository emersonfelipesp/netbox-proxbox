...
- Proxbox `0.0.26` is the current release for NetBox `4.5.8` through `4.5.10` and `4.6.x` (validated against `v4.5.8` through `v4.5.10` and `v4.6.0` through `v4.6.6`; declared stable range `4.5.8` through `4.6.99`), plus experimental evaluation on exact canonical NetBox `4.7.0-beta2`. The numeric `max_version` is `4.7.0`; a fail-closed canonical metadata guard rejects later prereleases and GA identities while allowing NetBox startup to omit the plugin. It pairs with `proxbox-api 0.0.20`, `proxmox-sdk 0.0.13`, and `netbox-sdk 0.0.10`.
- Upgrading to `0.0.26` applies migrations `0082` through `0087`: explicit Packer template-build authorization, OpenBao-first credential storage, the optional browser-console URL, and guarded retirement of obsolete reflection custom fields. Run the normal `manage.py migrate` and `collectstatic` steps. The field-retirement migrations preserve any definition with a meaningful stored value. Configure `console_url` only after the matching management frontend and backend relay are live at the trusted HTTPS origin; leaving it empty preserves the prior behavior. Existing backend rows still trust only their exact stored URL/IP, port, and TLS policy, and startup discovery without a row remains limited to configured or same-site targets derived from NetBox's trusted public origin.
...
> **Current release:** netbox-proxbox `0.0.26` pairs with proxbox-api `0.0.20` (NetBox `4.5.8`-`4.6.99` stable; exact canonical `4.7.0-beta2` experimental). Current backend-runtime pairing: netbox-proxbox 0.0.26 <-> proxbox-api 0.0.20 <-> proxmox-sdk 0.0.13 <-> netbox-sdk 0.0.10. This netbox-sdk version is proxbox-api's REST dependency only and does not provide the semantic MCP bridge.
