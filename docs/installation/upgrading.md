...
- Proxbox `0.0.23.post2` is the current release for NetBox `4.5.8` through `4.5.10` and `4.6.x` (validated against `v4.5.8` through `v4.5.10` and `v4.6.0` through `v4.6.5`; declared compatibility range `4.5.8` through `4.6.99`). It pairs with a `proxbox-api` guest-VM-interface writer build / next release. Plugin version `0.0.23.post2` on the `0.0.23` release line. The previous stable `0.0.22` release pairs with backend `0.0.19.post5`.
- Upgrading to `0.0.23.post2` enables bounded endpoint auto-configuration. Migration `0075_fastapi_backend_key_target_fingerprint` adds the durable credential-to-target binding, so run the normal `manage.py migrate` step. Existing backend rows trust only their exact stored URL/IP, port, and TLS policy; startup discovery without a row is limited to configured or same-site targets derived from NetBox's trusted public origin. An unproved target or unavailable local backend key remains pending rather than broadening discovery.
