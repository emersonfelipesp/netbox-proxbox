...
- Proxbox `0.0.17` is the current release for NetBox `4.5.8`, `4.5.9`, and `4.6.x` (certified simultaneously against `v4.5.8`, `v4.5.9`, `v4.6.0`, and official `v4.6.1`; declared compatibility range `4.5.8` through `4.6.99`). It is certified with the separate `proxbox-api` backend release `0.0.13`, including the matching `/proxmox/firewall/*` read surface and the `/intent/tag-pending-deletion` and `/intent/untag-pending-deletion` tag helpers. `proxbox-api` is not installed as a plugin dependency. The previous `0.0.16` release pairs with backend `0.0.12`.
