...
- Proxbox `0.0.13.post3` is the current release for NetBox `4.6.x` (certified against `v4.6.0-beta2`). It pairs with `proxbox-api==0.0.9.post2`, which fixes a `create_storages()` `TypeError` that affected `0.0.9.post1`. If you are still on NetBox `4.5.x`, stay on `0.0.11`.
