...
| Version | Summary |
|---------|---------|
| `0.0.15` | Decouples HTTPS scheme from `Verify SSL` on `FastAPIEndpoint` (issue #352); adds `overwrite_ip_address_dns_name` so the backend can populate `IPAddress.dns_name` from Proxmox guest hostnames (issue #354); ships a per-VM **HA tab** plus a cluster-wide **HA Status** page backed by new read-only HA endpoints (issue #243). Pairs with backend `0.0.11`. |
