...
| Version | Summary |
|---------|---------|
| `0.0.21` | Sync-mode filtering at source: per-record VM and VM-template filtering enforced by `proxbox-api 0.0.18.post3` using `sync_mode_vm`/`sync_mode_vm_template` query params — a `disabled` mode no longer creates dependent NetBox objects for skipped VMs. Two-phase VM batch processing and per-VM dispatch failure isolation. Interface-dense guest alias matching by name and partial-failure stream frame. Pairs with backend [`proxbox-api 0.0.18.post3`](https://github.com/emersonfelipesp/proxbox-api), `proxmox-sdk 0.0.12`, and `netbox-sdk 0.0.10`. |
