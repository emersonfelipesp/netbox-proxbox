...
| Version | Summary |
|---------|---------|
| `0.0.15.post2` | Post-release patch on stable `0.0.15`: adds the `build-pve-template` REST action on `ProxmoxEndpointViewSet` (#452) and pairs with backend `proxbox-api 0.0.11.post2`, which lifts the `proxmox-sdk` pin to `0.0.5.post1` so the new `proxmox_sdk.pdm` subpackage is available to the backend. Foundation cut before the plugin-side PDM model + UI work tracked in [#449](https://github.com/emersonfelipesp/netbox-proxbox/issues/449). No NetBox compatibility rotation. |
