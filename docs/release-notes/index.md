...
The plugin source in this repository is currently `0.0.27rc2` on the
...
| Version | Summary |
|---------|---------|
| `0.0.27rc2` | Exports VM list filter helpers from `netbox_proxbox.utils` and ships production deploy compatibility with the `proxbox-api 0.0.22` pairing. |
...
> **Current release:** netbox-proxbox `0.0.27rc2` pairs with proxbox-api `0.0.22` (NetBox `4.5.8`-`4.7.0`, including official v4.7.0 GA). Current backend-runtime pairing: netbox-proxbox 0.0.27rc2 <-> proxbox-api 0.0.22.post1 <-> proxmox-sdk 0.0.13 <-> netbox-sdk 0.0.13. This netbox-sdk version is proxbox-api's REST dependency only and does not provide the semantic MCP bridge.
