...
The plugin source in this repository is currently `0.0.17` on the `v0.0.17` branch. The sibling plugins (`netbox-pbs`, `netbox-ceph`, `netbox-pdm`, `netbox-packer`) live in standalone repositories under [@emersonfelipesp](https://github.com/emersonfelipesp); each declares `netbox-proxbox>=0.0.16` as an install-time dependency and reuses the shared `proxbox-api` backend.
