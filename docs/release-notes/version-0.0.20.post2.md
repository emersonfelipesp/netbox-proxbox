# Version 0.0.20.post2

netbox-proxbox `0.0.20.post2` pairs with the `proxbox-api 0.0.17.post1`
...
- **Latest Sync Jobs on the plugin homepage.** The Proxbox homepage now shows a
  read-only **Latest Sync Jobs** table with the five most recent Proxbox sync
  jobs, rendered after the additional plugin endpoint cards. A **View all sync
  jobs** button links operators to the full sync-job list when they need more
  history.
...
- Upgrade the plugin to `netbox-proxbox 0.0.20.post2` and keep the backend on
...
- No model, migration, dependency, NetBox compatibility, or backend-contract
  changes are included in this post-release patch.
