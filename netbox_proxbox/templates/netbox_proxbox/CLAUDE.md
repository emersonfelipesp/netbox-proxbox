...
- Inventory detail/list pages: `storage_list.html`, `vmbackup.html`, `vmbackup_list.html`, `vmbackup_bulk_delete.html`, `vmsnapshot.html`, `vmsnapshot_list.html`, `vmtaskhistory.html`, `proxmoxstorage.html`, `backup_routine.html`, `backup_routine_list.html`, `replication.html`, `replication_list.html`, `soft_deleted_virtual_machines.html` (the marker-filtered human-only VM purge page), `vm_proxmox_config.html` (live Proxmox config tab), and `vm_console.html` (the standalone, permission-gated core VM Console tab). The console template receives only a server-owned session URL, CSRF token, readiness state, VM type, and static asset locations; it must never render Proxmox or proxbox-api credentials, tickets, or topology.
