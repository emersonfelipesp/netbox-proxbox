...
**Object tab strip on edit sub-pages.** `proxmoxendpoint_settings.html` and `proxmoxendpoint_ssh_settings.html` are NetBox `ObjectEditView`s, so `generic/object_edit.html` would render `{% block tabs %}` as a lone **Edit** tab, hiding the object's other registered tabs. Both templates override `{% block tabs %}` (with `{% load helpers %}{% load tabs %}`) to render the full object tab strip like `generic/object.html` — the primary detail tab (`object.get_absolute_url`) plus `{% model_view_tabs object %}` (navigable `<a href>` per `ViewTab`: Settings / SSH / Terminal / Sync Jobs / Templates …). The current tab highlights via `context['tab']` (set to `self.tab` by `ObjectEditView.get_extra_context`); the edit form still renders because the content pane is hardcoded `tab-pane show active`. Guarded by `tests/test_frontend_contracts.py::test_proxmox_endpoint_edit_subpages_keep_object_tab_strip`.
