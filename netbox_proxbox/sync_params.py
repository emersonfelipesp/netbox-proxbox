...
def effective_tenant_tag_assignment_for_endpoint(
    proxmox_endpoint_id: int | str | None,
) -> bool:
    """Resolve tenant tag-assignment enablement for a given endpoint."""
    try:
        from netbox_proxbox.models import ProxboxPluginSettings
    except (ImportError, RuntimeError):
        return False
    try:
        ...
        return False
    enabled = bool(getattr(settings_obj, "enable_tenant_tag_assignment", False))
    ...
    if endpoint is None:
        return enabled
    ep_enabled = getattr(endpoint, "enable_tenant_tag_assignment", None)
    if ep_enabled is not None:
