...
| `ProxboxPluginSettingsViewSet` | `settings/` | GET+PATCH only (singleton); the retired external browser-console URL is not part of the settings API. `encryption_key` is write-only on ordinary serializers, ordinary key mutation is rejected while ciphertext exists, all validation/mutation entry frames are redact-all for exception reports, and `/runtime/` retains the existing permission-gated key response for current proxbox-api compatibility plus `encryption_key_configured`. Remove the fallback only with a paired backend migration. See `../../docs/features/browser-console.md` for the standalone console contract. |
