...
class DashboardView(
    ...
    def _endpoint_query_params(self, endpoint: ProxmoxEndpoint) -> dict[str, str]:
        ...
        return {"source": "database", "ip_address": str(endpoint.ip_address).split("/")[0]}
    ...
    def _build_cluster_summary(self, cluster_payload: Any) -> dict[str, Any]:
        ...
        if online_nodes == 0:
            online_nodes = sum(1 for r in node_records if str(r.get("status")) == "online")
    ...
    def _build_guest_summary(self, resources_payload: Any) -> dict[str, dict[str, int]]:
        ...
        return {"virtual_machines": _count(qemu_records), "lxc_containers": _count(lxc_records)}
