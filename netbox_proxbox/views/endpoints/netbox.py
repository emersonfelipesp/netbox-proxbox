...
class NetBoxEndpointExportView(generic.ObjectListView):
    """Download filtered NetBox endpoints as CSV, JSON, or YAML; secrets require token proof."""
    ...
    def _validate_sensitive_export_token(self, request: HttpRequest) -> bool:
        """Confirm POSTed NetBox API token maps to a user allowed to view NetBox endpoints.
        ...
        if not user.has_perm("netbox_proxbox.view_netboxendpoint"):
            messages.error(
                request,
                "The provided token user does not have permission to view NetBox endpoints.",
            )
            return False
    ...
    def _export_response(
        ...
        rows = [
            _serialize_netbox_endpoint(endpoint, include_sensitive)
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if include_sensitive and not self._validate_sensitive_export_token(request):
            return redirect("plugins:netbox_proxbox:netboxendpoint_list")
        return self._export_response(
            request,
...
class NetBoxExportQuickAddTokenView(View):
