...
- [`data_protection.py`](./data_protection.py): shared list-view calendar mixin
  and the combined Data Protection page. The list mixin consumes the filtered,
...
- [`vm_console.py`](./vm_console.py): registers the standalone Console tab on core `VirtualMachine` detail pages and creates opaque proxbox-api relay sessions. It authorizes the restricted VM and exact synchronized `ProxmoxEndpoint` before any backend request, sends the API key only server-side, disables redirects, and returns only a validated WSS URL, expiry, and console type to the browser. The tab view deliberately overrides `ObjectPermissionRequiredMixin.has_permission()` while retaining its `get_queryset()` restriction: the required permission belongs to `ProxmoxEndpoint`, and allowing NetBox to derive that `open_console` action for the `VirtualMachine` queryset makes every authorized VM lookup return 404. The override preserves the inherited required-plus-`additional_permissions` check without rewriting the queryset; `get_queryset()` independently enforces exact VM visibility, and `get_extra_context()` checks the synchronized endpoint object restriction. See `../../docs/features/browser-console.md`.
...
- [`proxmox_create_instance.py`](./proxmox_create_instance.py): POST-only registered ProxmoxEndpoint action (`path="create-instance"`) for the Templates-tab wizard. Requires `core.run_proxmox_action`, validates JSON server-side, pre-checks `allow_writes`, calls proxbox-api directly for QEMU/LXC provisioning, retries QEMU VMID collisions, surfaces proxbox-api 403 reasons verbatim, and performs best-effort single-object VM sync-back. That sync-back first enforces the shared branch-isolation guard, then passes the active branch schema ID, the already selected FastAPI endpoint ID, and the exact resolved backend Proxmox endpoint ID to `sync_individual()`; a missing isolation boundary or backend pin leaves the sync pending and never widens the mutation.
...
- [`settings.py`](./settings.py): plugin settings page for runtime feature
  ...
  requires normal settings-change permission; reset requires a NetBox Object
  Permission on `ProxboxPluginSettings` with the additional action
  `reset_encrypted_secrets` and is POST-only. Rotation marks all
