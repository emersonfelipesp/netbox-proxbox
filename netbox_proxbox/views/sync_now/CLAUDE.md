...
- `VMTaskHistory.vm_type` is trusted only after normalization to `qemu` or `lxc`. The resolver derives invalid/default/blank legacy values from the linked VM, resolves VMID from `virtual_machine.proxbox_sync_state.proxmox_vm_id` before the deprecated custom fields, and returns a local 422 when required context remains unknown; `type=unknown` and unscoped VMIDs must never reach proxbox-api.
- Snapshot requests validate and prefer the row's authoritative `VMSnapshot.subtype`. Invalid or missing subtype values may fall back to the linked VM's typed sync state, native VM type, and finally transition-era custom fields, but never to the generic QEMU compatibility default; no supported type means a local 422.
