...
- `VMTaskHistory.vm_type` is trusted only after normalization to `qemu` or `lxc`. The resolver derives invalid/default/blank legacy values from the linked VM, and returns a contextual local 422 when neither source identifies a supported type; `type=unknown` must never reach proxbox-api.
