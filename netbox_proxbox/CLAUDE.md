...
- [`redaction.py`](./redaction.py): the shared, dependency-free vocabulary of credential-bearing field markers and HTTP authentication schemes, imported by **both** `anonymize.py` (public report) and `views/error_utils.py` (job log). `SENSITIVE_KEY_MARKERS` (normalised, for key comparison) and `KEY_MARKER_PATTERN` (regex fragment, for raw text) are generated from one `_MARKER_WORDS` list so the two spellings cannot drift. **Must import nothing but `re`** — `anonymize` needs it without Django, and `error_utils` sits behind a package that requires Django.
