...
- [`plugin_settings.py`](./plugin_settings.py): singleton plugin settings model. It does not carry an external console URL; migration `0095` retires that handoff setting. The standalone console resolves an enabled `FastAPIEndpoint` server-side as documented in `../../docs/features/browser-console.md`.
