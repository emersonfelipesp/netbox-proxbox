...
## What's New in v0.0.27rc14

Current source pairing: netbox-proxbox 0.0.27rc14 <-> proxbox-api 0.0.23.post1 <-> proxmox-sdk 0.0.15 <-> netbox-sdk 0.0.13. This is the current sibling-source development stack, not a rewrite of historical release compatibility. This netbox-sdk version is proxbox-api's REST dependency only and does not provide the semantic MCP bridge.
...
- **Canonical sync redirect validation.** Release validation accepts the real
  Proxbox home redirect, rejects ambiguous redirect and identifier forms, and
  verifies every polled Core Job resource against the authoritative response
  header ID.
- **Authoritative E2E sync-job correlation.** Successful Proxbox sync enqueue
  responses expose the exact NetBox Core Job ID, and release validation polls
  only that record under one monotonic deadline. This prevents cache-update
  child jobs or concurrent same-name syncs from hiding or impersonating the
  triggered job.
- **Mock-safe endpoint transport budget.** Disposable Page Coverage and Docker
  E2E endpoints use an explicit 60-second Proxmox request timeout with one
  bounded retry, avoiding hosted mock-server timeouts without changing
...
- **Reliable disposable-stack validation.** The Page Coverage fixture selects
  local encrypted credential storage on its singleton settings before creating
  endpoint credentials, so the fixture remains self-contained while production
...
- **Proxbox OCI testing appliance.** Builds a digest-pinned official NetBox image with dynamically resolved stable netbox-proxbox and proxbox-api releases, automatic plugin configuration, localhost backend communication, persistent services under `/sbin/init`, and direct `manage.py createsuperuser` administration for Proxmox OCI-to-LXC testing.
...
|--------|----------------|-------------|------------|-------------|
| 4.5.8-4.7.0 GA (current source) | v0.0.27rc14 | v0.0.23.post1 | v0.0.13 | v0.0.15 |
