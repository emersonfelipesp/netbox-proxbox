...
def test_release_index_declares_current_release_exactly():
    text = _read(REPO_ROOT / "docs" / "release-notes" / "index.md")
    assert (
        f"The plugin source in this repository is currently `{CURRENT_RELEASE_VERSION}`"
        in text
    )
    assert (
        f"> **Current release:** netbox-proxbox `{CURRENT_RELEASE_VERSION}` pairs"
        in text
    )
    stale = re.findall(
        r"currently `([0-9][^`]*)`|Current release:\*\* netbox-proxbox `([0-9][^`]*)`",
        text,
    )
