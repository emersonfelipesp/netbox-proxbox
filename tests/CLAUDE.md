...
assignment for unlinked nodes, link/relink/unlink reconciliation, foreign
primary conflict rollback, immutable UUID references, actor and provider
failure rollback, fail-closed OpenBao reads without Fernet fallback, owner
deletion cleanup, shared-credential assignment isolation, authenticated UI and
REST transaction ordering/actor propagation for edit, delete, bulk, and node
link paths, raw/bulk/cascade/downgrade guards, provider-removal diagnostics,
secret-free readiness and Ansible selectors, authenticated hardware-consumer
resolution, traceback redaction, and migration `0100` state/data round trips.

`test_openbao_single_secret_contracts.py` and
`test_openbao_single_secret_django.py` cover the typed single-secret owner
contract for FastAPI, PBS, PDM, and Firecracker host tokens. Keep the
`api-token` type, `api`/`agent` purposes, idempotent migration `0101`,
write-only inputs, provider transaction boundary, fail-closed reads, assignment
reconciliation, shared-safe deletion, and downgrade refusal aligned across the
source and exact-provider lanes.

`test_openbao_cloudinit_contracts.py` and
`test_openbao_cloudinit_django.py` cover migration 0102, write-only password
and private-key inputs, VM `login` assignments, `ssh_pwauth` primary selection,
strict provider resolution, legacy-reference preservation, provider-first API
transaction ordering, authenticated permissions, CAS/concurrent rotation,
rollback/retry, shared-safe cleanup, provider absence, traceback redaction,
VM reassignment, authenticated bulk mutation, and raw/bulk/cascade/downgrade
guards. The migration test reverses and reapplies 0102 without losing UUID or
legacy-reference data. Public SSH-key reflection and intent fields must remain
outside every provider payload.
...
The branch activation and lifecycle tests use unit-level fakes modeled on the
installed `netbox-branching` `1.2.0-beta1` API, including its no-change merge
remaining in `READY`. Real Django/PostgreSQL integration with
...
- `test_data_protection_calendar.py` and `test_pve_calendar_event.py` path-load
  the pure calendar and schedule modules without bootstrapping Django. They
  cover month/week boundaries, overflow, preserved query parameters, schedule
  grammar, fail-open handling, and hostile input.
- `test_data_protection_views.py` pins the view, URL, navigation, template,
...
- `test_dependency_security_floors.py`: guards the declared dependency floors that carry a security meaning, which `uv.lock` cannot protect — the lock is neither shipped in the wheel nor consulted by `pip`, so a floor admitting an advisory-covered version silently re-introduces the vulnerability on the next fresh resolve while the lockfile scanner reports nothing. Each `SECURITY_FLOORS` entry names the manifest positions that must declare it (`declared_in`), and every one is checked: `cryptography>=50.0.0` in the shipped runtime dependencies; `aiohttp` in the shipped `cli` extra and `requirements-docs.txt`; `pymdown-extensions` in `[dependency-groups].dev` and `requirements-docs.txt`; and `mkdocs-material>=9.7.7` in both documentation manifests. `pymdown-extensions` is only ever *transitive* — `mkdocs-material` asks merely for `>=10.2`, which admits both GHSA-gm37-52c6-37mw (`<=11.0.0`) and GHSA-9xwg-3r6f-jcx2 (`<=10.21.3`) — and `.github/workflows/docs.yml` installs the docs requirements with a plain `pip install -r` that reads no lockfile, so an **undeclared** package is a real exposure and the guard treats "not declared here" as a failure rather than a skip. The lock test pins the inverse mistake: a declared floor raised while `uv.lock` stays below it, which makes `uv sync --locked` inconsistent. The same module also guards every reviewed NetBox matrix `.in` and hash-locked `.txt` pair against the Django, Django REST Framework, mkdocs-material, Pillow, PyJWT, Strawberry GraphQL, and tablib advisory floors. It fails closed on opaque input directives, direct URLs, environment markers, missing or duplicate package pins, missing artifact hashes, prerelease substitutions, and any direct input requirement whose generated lock pin is absent or does not satisfy the declaration. Every non-comment top-level lock install line must be an exact index pin; direct URL, VCS, local-path, alternate-index, and find-links forms fail closed. Lane-specific authentication-chain assertions protect the coherent PyJWT, requests, social-auth-app-django, and social-auth-core pins, and mutation cases prove that a stale non-floor lock, an inactive marker, a direct-URL input substitution, a direct-URL lock substitution, and an alternate-index directive are rejected.
...
- `test_stage_failure_cause.py`: behavior coverage for issue #383's retry-history diagnosis. It proves every failed attempt is recorded with typed attribution data, the first application failure wins over later transport noise in either order, all-transport failures use the final attempt, identical details preserve the legacy single-line message, divergent details are bounded, non-retryable failures remain byte-compatible, the final error log matches the raised message, and the `init_ok` readiness diagnosis still fires.
- `test_sync_types_stage_failure.py`: direct compatibility guards for `_compose_stage_failure_message()`, preserving `_format_stage_sync_error()`'s exact PostgreSQL connection-slot guidance and exception-name suffix.
...
- `test_release_workflows.py` also owns the **public-disclosure guard** (`test_public_files_name_no_private_infrastructure`): it diffs the branch against its review base and fails on any added line naming private infrastructure. The base is resolved fail-closed by `_review_base()` **per event**: a `pull_request` run uses its base branch (`PROXBOX_REVIEW_BASE`, then `gitea/<base>`, then `origin/<base>` with `<base>` = `GITHUB_BASE_REF` or `develop`); a branch `push` uses the event's `before` commit, which must exist and be an ancestor of `HEAD` (the branch ref itself is `HEAD` after the push, so a merge base against it would be empty); a tag push, `release`, `workflow_dispatch`, or `schedule` run has no branch base and skips with that reason, because the change it validates was scanned on its pull request and on the push that landed it. A missing pull-request base or a failing `git diff` **fails** the test under `CI` (or `GITHUB_ACTIONS`/`GITEA_ACTIONS`); only a local checkout without any remote may skip. The untrusted Gitea runner exports none of those markers (probe in run 24064), so the Gitea quality job sets `CI`, `GITHUB_EVENT_NAME`, `GITHUB_BASE_REF`, `GITHUB_REF`, and — for a branch push — `PROXBOX_REVIEW_BASE` from `github.event.before` explicitly; `test_gitea_quality_job_tells_the_guard_it_runs_in_ci` pins that. Both mocked-suite CI checkouts therefore use `fetch-depth: 0`, pinned by `test_mocked_suite_checkouts_fetch_the_review_base`. Added, modified, or renamed **binary** public files are refused outright — the line scanner cannot read them, and a directory name is not provenance (any bytes can be put under any path in a pull request); regenerated screenshots reach `develop` through the screenshots workflow's own `[skip ci]` push, which carries no pull request. Changed paths come from `git diff --numstat -z`, so a filename containing a tab or newline, or a rename shown as `old => new` in the human form, cannot hide a file; each text file is then diffed individually and its quoted patch header ignored. A zero-addition diff skips with a reason (identical trees, or removals only). `test_module_layout.py` pins that the removed legacy services and the shadowed `utils.py` module do not return in the source tree, and `scripts/check_dist_inventory.py` — run by both CI package builds, pinned by `test_ci_builds_run_the_dist_inventory_check` — refuses a wheel or sdist that still ships them.
...
- `test_version.py` pins the combined NetBox 4.7 companion cell: immutable
  netbox-ceph, netbox-pbs, netbox-pdm, and netbox-packer sources; checked-out
  HEADs and reviewed pyproject digests; the composed hash-locked dependency
  artifact; editable no-dependency installation; and post-migration assertions
  that NetBox registers all five plugins without plugin system-check warnings.
  Do not replace this with import-only coverage or a mutable branch checkout.
...
- `test_node_ssh_credential_api.py`: `_NetBoxTokenCanViewNodeSSHCredential.has_permission()` NetBox-token permission checks, `_credential_for_node_identifier()` ProxmoxNode/NetBox-device lookup compatibility, `_metadata_payload()` redaction, AST contract on the by-node viewset (`_ProxboxDashboardPermission` for metadata, NetBox-token permission for secrets, HTTPS-required guard in non-DEBUG, 503 on missing encryption key), and `api/urls.py` route registration. Also **behavioral** coverage of `NodeHostKeyFingerprintAPIView.get` (the Terminal-tab node host-key scan) via the stub loader: `ssh_access_enabled` 403 gate, no-IP 422, no-backend 503, success forwards `host`/`port` to proxbox-api and returns the fingerprint, invalid `?port=` defaults to 22, and old-backend 404→503 downgrade. Also covers `NodeSSHCredentialSecretsAPIView._resolve_from_device_openbao()` (issue #614): a `NodeSSHCredential.DoesNotExist` on the primary lookup defers to `device_openbao_ssh_resolver.resolve_node_ssh_from_device_openbao()`, a valid `?port=` is parsed and forwarded while an invalid one is dropped to `None`, a returned payload is passed through as `200`, `None` becomes the existing `404` "no local credential" body, and a raised `PermissionDenied` becomes `403` with its message and no traceback. `_proxmox_node_for_identifier()` mirrors `_credential_for_node_identifier()`'s ProxmoxNode-PK-first, linked-device-PK-second resolution — proven with a node whose own pk (42) differs from its linked device's id (4242) so a stub that ignored the lookup field could not pass vacuously — and fails closed (`404`, "Multiple Proxmox nodes…") rather than picking one when two `ProxmoxNode` rows share one device id; the fake `_NodeQuerySet.get()` used to prove this raises real `DoesNotExist`/`MultipleObjectsReturned` semantics rather than always returning a single fixed node. **Identifier collision (round 2):** both `_credential_for_node_identifier()` and `_proxmox_node_for_identifier()` now always resolve *both* interpretations of `node_id` before returning, and `test_credential_lookup_refuses_a_pk_versus_device_id_collision()` / `test_resolve_from_device_openbao_refuses_a_pk_versus_device_id_collision()` prove that when `node_id` is simultaneously one row's own PK and a *different* row's linked-device PK, the lookup raises `_AmbiguousNodeIdentifier` (surfaced as `404`, never silently falling through to the OpenBao path) rather than preferring the PK match. The counterpart tests (`…_pk_match_wins_over_an_unrelated_multi_device_match` / `…_pk_match_wins_over_unrelated_collision`) prove the opposite is *not* broken: unrelated nodes that happen to coincidentally share this value as a device id among **themselves** (`MultipleObjectsReturned` from the device-id lookup alone) must not block an otherwise clean, unambiguous PK match — only a genuine disagreement between the two interpretations is a collision.
- `test_device_openbao_ssh_resolver.py`: real path-loaded (not AST-only) behavior coverage for `netbox_proxbox/api/device_openbao_ssh_resolver.py` (issue #614), stubbing only `django.apps`/`django.contrib.contenttypes`/`django.core.exceptions` since every netbox-openbao model is resolved through `apps.get_model()` rather than imported at module load time. Covers the SSH-access gate (a disabled owning `ProxmoxEndpoint` denies before any lookup runs; no owning endpoint is not gated), that netbox-openbao being uninstalled or the node having no linked device both return `None` rather than raising, and that a match is revealed directly with no further fallback. `_match_openbao_endpoint()` fails closed (`PermissionDenied`, without logging any credential/material) on more than one credentialed SSH endpoint and is narrowed to exactly one by an explicit port. `_reveal_openbao_endpoint()` verifies the credential's identity with an *independent* `apps.get_model("netbox_openbao", "Credential")` lookup keyed by `endpoint.credential_id` that also requires the reverse `service_endpoints` relation back to this exact endpoint — not a trust of a possibly stale cached `endpoint.credential` — and credential type before trusting `reveal_credential_material()`'s payload; shapes password vs. keypair responses (`auth_method` mapped to the existing `"password"`/`"key"` contract, not netbox-openbao's own `credential_type` spelling); and never lets a provider exception's text escape the raised, generic `PermissionDenied`. There is no legacy-plugin fallback path in this module — see `CLAUDE.md`/`AGENTS.md` for why one is not present. **Feature detection (round 2):** `_get_openbao_model()` converts `apps.get_model()`'s `LookupError` — real Django behavior for a model an installed app doesn't define, reproduced in the fake `_FakeApps.get_model()` rather than a fictitious stand-in exception — into "not available"; `test_get_openbao_model_returns_none_for_a_missing_model`, `test_match_openbao_endpoint_returns_none_when_service_endpoint_is_undefined`, `test_resolve_node_ssh_returns_none_when_service_endpoint_is_undefined`, and `test_reveal_openbao_endpoint_returns_none_when_credential_is_undefined` cover this at each layer, against an empty `models={}` registry — exactly the shape of the pinned CI netbox-openbao revision, which predates `ServiceEndpoint` (see `docs/companion-plugins/netbox-openbao.md`). **Authorization**: the fake `Credential.objects` queryset asserts `restrict(user, "reveal")` is called before `filter()`/`first()` (an `AssertionError` if not, so a regression that drops the check cannot pass silently) and honors an explicit `authorized_users` allowlist mirroring NetBox's `RestrictedQuerySet`. An unauthenticated caller (`user=None`) is denied before any queryset call; a caller authenticated but outside the allowlist — modeling one holding only `view_nodesshcredential` — is denied; and an authorized caller succeeds, proving the gate discriminates rather than always denying or always passing.
