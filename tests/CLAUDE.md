...
- `test_node_ssh_credential_api.py`: `_ProxboxBackendBearer.has_permission()` token-matching, `_metadata_payload()` redaction, AST contract on the by-node viewset (`_ProxboxDashboardPermission` for metadata, `_ProxboxBackendBearer` for secrets, HTTPS-required guard in non-DEBUG, 503 on missing encryption key), and `api/urls.py` route registration for both endpoints.
