...
The current plugin config lives in [`netbox_proxbox/__init__.py`](./netbox_proxbox/__init__.py). It declares plugin version `0.0.21` (release candidate `0.0.21`) and NetBox compatibility `4.5.8` through `4.6.99` (validated against `4.5.8`, `4.5.9`, `4.6.0`, and official `4.6.1`). The current pairing is `netbox-proxbox 0.0.21` ↔ `proxbox-api 0.0.18` ↔ `proxmox-sdk 0.0.12` ↔ `netbox-sdk 0.0.10`. The `0.0.21` release enforces per-record VM/VM-template sync-mode filtering at the proxbox-api backend source, two-phase VM batch processing, and per-VM dispatch failure isolation. The previous stable `0.0.20.post3` release pairs with backend `0.0.17.post1`; `0.0.20` pairs with `0.0.17`. `proxbox-api` is not a Python dependency of this plugin; the services communicate over HTTP.
