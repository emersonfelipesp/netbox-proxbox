...
The current plugin config lives in [`netbox_proxbox/__init__.py`](./netbox_proxbox/__init__.py). It declares plugin version `0.0.22` and NetBox compatibility `4.5.8` through `4.6.99` (validated against `4.5.8`, `4.5.9`, and `4.6.0` through `4.6.4`). Current pairing: `netbox-proxbox 0.0.22 ↔ proxbox-api 0.0.19 ↔ proxmox-sdk 0.0.12 ↔ netbox-sdk 0.0.10`. The `0.0.22` release documents the full Proxmox SDN sync line: optional `sync_mode_sdn`, plugin SDN inventory, and backend reconciliation into NetBox L2VPN, RouteTarget, Prefix, and L2VPNTermination objects. SDN sync requires `proxbox-api >= 0.0.19`. The previous plugin release `0.0.21` pairs with backend `0.0.18.post5`; `0.0.20.post3` pairs with backend `0.0.17.post1`; `0.0.20` pairs with `0.0.17`. `proxbox-api` is not a Python dependency of this plugin; the services communicate over HTTP.
