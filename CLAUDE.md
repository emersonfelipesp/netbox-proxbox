...
The current plugin config lives in [`netbox_proxbox/__init__.py`](./netbox_proxbox/__init__.py). It declares plugin version `0.0.15.post2` and NetBox compatibility `4.5.8` through `4.6.99` (certified simultaneously against `4.5.8`, `4.5.9`, and official `4.6.0`). The `0.0.15.post2` release is certified with the separate `proxbox-api` backend release `0.0.11.post2`, which lifts the `proxmox-sdk` pin to `0.0.5.post1` (adding the new `proxmox_sdk.pdm` subpackage) and adds the `build-pve-template` REST action on `ProxmoxEndpointViewSet`. The `0.0.15.post1` release pairs with backend `0.0.11.post1`. `proxbox-api` is not a Python dependency of this plugin; the services communicate over HTTP.
