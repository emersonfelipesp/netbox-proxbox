...
The current plugin config lives in [`netbox_proxbox/__init__.py`](./netbox_proxbox/__init__.py). It declares plugin version `0.0.17` and NetBox compatibility `4.5.8` through `4.6.99` (certified simultaneously against `4.5.8`, `4.5.9`, `4.6.0`, and official `4.6.1`). The `0.0.17` release is certified with the separate `proxbox-api` backend release `0.0.13` and adds read-only Proxmox VE firewall sync for every PVE firewall zone (datacenter, node, per-VM QEMU + LXC, per-VNet SDN) per [#326](https://github.com/emersonfelipesp/netbox-proxbox/issues/326); firewall objects are reflected through the backend `/proxmox/firewall/*` routes and not yet persisted as Django models, so there is no DB migration. The previous `0.0.16` release pairs with backend `0.0.12`. `proxbox-api` is not a Python dependency of this plugin; the services communicate over HTTP.
