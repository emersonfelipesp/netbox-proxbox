...
The current plugin config lives in [`netbox_proxbox/__init__.py`](./netbox_proxbox/__init__.py). It declares plugin version `0.0.14` and NetBox compatibility `4.5.8` through `4.6.99` (certified simultaneously against `4.5.8`, `4.5.9`, and official `4.6.0`). The `0.0.14` release is certified with the separate `proxbox-api` backend release `0.0.10.post2` (certification bump over `0.0.13.post4` / backend `0.0.9.post2`; no plugin code changes — REST/SSE/WebSocket/auth/overwrite-flag contracts remain compatible). `proxbox-api` is not a Python dependency of this plugin; the services communicate over HTTP.
