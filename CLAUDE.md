...
The current plugin config lives in [`netbox_proxbox/__init__.py`](./netbox_proxbox/__init__.py). It declares plugin version `0.0.24` and sources its NetBox bounds from [`netbox_proxbox/compat.py`](./netbox_proxbox/compat.py), which defines two tiers: a **stable** band `4.5.8` through `4.6.99` (certified; validated against `4.5.8`-`4.5.10` and `4.6.0`-`4.6.6`) and an **experimental** band `4.7.0` through `4.7.99` (loads and runs with no configuration, warns once via system check `netbox_proxbox.W001`). `min_version`/`max_version` are `PLUGIN_MIN_VERSION`/`PLUGIN_MAX_VERSION` from that module — `max_version` is the experimental ceiling `4.7.99`. `compat.py` is vendored byte-identically across netbox-proxbox, netbox-ceph, netbox-packer, netbox-pbs, and netbox-pdm; change it in one repo and you must change it in all five. It must not import Django at module scope, because NetBox imports it while `netbox/settings.py` is still executing. Current backend-runtime pairing: netbox-proxbox 0.0.24 <-> proxbox-api 0.0.20 <-> proxmox-sdk 0.0.13 <-> netbox-sdk 0.0.10. This netbox-sdk version is proxbox-api's REST dependency only and does not provide the semantic MCP bridge. The `0.0.24` release adds NetBox 4.6.6 certification, settings/storage compatibility fixes, blank-key encryption recovery, and immutable Gitea-first release provenance while retaining bounded endpoint auto-configuration and the universal `guest_os_model` behavior. Existing backend rows authorize only their exact persisted target; rowless discovery is restricted to configured or same-site targets derived from NetBox's trusted public origin, and any unproved target remains pending. The previous stable `0.0.23.post2` release introduced bounded endpoint auto-configuration. `proxbox-api` is not a Python dependency of this plugin; the services communicate over HTTP.
...
- **Stage failure reporting (`sync_stages.StageOutcome`, `jobs._describe_stage_failure`).** A sync stage is no longer called successful on HTTP status alone. The backend's SSE generator sets the terminal `complete` frame's `ok` from "the sync coroutine returned without raising", which a coroutine that reconciled 0 of N objects does — so a run in which NetBox rejected every write used to finish `completed` with a green stage summary and the errors visible only in the backend log. `StageOutcome` folds the per-object frames the stream already carried into a per-stage tally, fed from the existing `on_frame()` callback so frame inspection stays in one place. Evidence precedence is strict and single-source: the terminal `phase_summary` frame's `result` counters when present, else failed `item_progress` frames, else `error_detail` frames — never summed, so a stage emitting several is not double-counted. Verdict is three-valued: `success` (no failures), `warning` (some failed, some succeeded — the run still completes but logs the counts, because one unreachable guest must not fail an estate-wide scheduled sync), `failed` (failures and **zero** successes — the run errors). `failed` additionally requires *countable* evidence: an `error_detail`-only stage has no denominator and several backend paths emit one non-fatally, so it caps at `warning` — replacing a false green with a false red is not an improvement. `_execute_stage_sync()` calls `outcome.reset()` before **each retry attempt**, because every attempt replays its own frames through the same callback; without it a transient 503 that succeeded on retry stayed marked failed with both attempts' counts summed. The error is raised **after** `self.job.save(update_fields=["data"])`, mirroring the endpoint-scope fail-loud ordering, so the per-stage counts survive on the errored row. `result_summary` gains `status`, `failed`, `succeeded`, and up to three deduplicated `errors` samples; `path` and `ok` keep their existing meaning. A fresh accumulator is bound per stage **and per stream path**, so counts never leak between stages or endpoints. All frame reads are total: `_as_count()` coerces strings/floats/negatives/`None` and rejects booleans, and `observe()` swallows malformed frames rather than aborting a sync that is otherwise succeeding. Contracts: `tests/test_stage_failure_reporting.py`, `tests/test_jobs.py`.
- **Cluster/node counts are plugin records, not NetBox objects.** `sync_cluster_and_nodes()` counts `ProxmoxCluster`/`ProxmoxNode` rows written by `update_or_create()` in the plugin's own tables. The job log says so explicitly; the previous wording read as though NetBox had been populated, which is how a run that wrote no NetBox objects at all could look successful.
